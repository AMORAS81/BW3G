; item ids
; indexes for:
; - ItemNames (see data/items/names.asm)
; - ItemDescriptions (see data/items/descriptions.asm)
; - ItemAttributes (see data/items/attributes.asm)
; - ItemEffects (see engine/items/item_effects.asm)
	const_def
	const NO_ITEM      ; 00
	const MASTER_BALL  ; 01
	const ULTRA_BALL   ; 02
	const BRIGHTPOWDER ; 03
	const GREAT_BALL   ; 04
	const POKE_BALL    ; 05
	const TOWN_MAP     ; 06
	const BICYCLE      ; 07
	const MOON_STONE   ; 08
	const ANTIDOTE     ; 09
	const BURN_HEAL    ; 0a
	const ICE_HEAL     ; 0b
	const AWAKENING    ; 0c
	const PARLYZ_HEAL  ; 0d
	const FULL_RESTORE ; 0e
	const MAX_POTION   ; 0f
	const HYPER_POTION ; 10
	const SUPER_POTION ; 11
	const POTION       ; 12
	const ESCAPE_ROPE  ; 13
	const REPEL        ; 14
	const MAX_ELIXIR   ; 15
	const FIRE_STONE   ; 16
	const THUNDERSTONE ; 17
	const WATER_STONE  ; 18
	const DAWN_STONE   ; 19
	const HP_UP        ; 1a
	const PROTEIN      ; 1b
	const IRON         ; 1c
	const CARBOS       ; 1d
	const HEART_SCALE  ; 1e
	const CALCIUM      ; 1f
	const RARE_CANDY   ; 20
	const X_ACCURACY   ; 21
	const LEAF_STONE   ; 22
	const METAL_POWDER ; 23
	const NUGGET       ; 24
	const POKE_DOLL    ; 25
	const FULL_HEAL    ; 26
	const REVIVE       ; 27
	const MAX_REVIVE   ; 28
	const GUARD_SPEC   ; 29
	const SUPER_REPEL  ; 2a
	const MAX_REPEL    ; 2b
	const DIRE_HIT     ; 2c
	const SHELL_STONE  ; 2d
	const FRESH_WATER  ; 2e
	const SODA_POP     ; 2f
	const LEMONADE     ; 30
	const X_ATTACK     ; 31
	const BOARDINGPASS ; 32
	const X_DEFEND     ; 33
	const X_SPEED      ; 34
	const X_SPECIAL    ; 35
	const COIN_CASE    ; 36
	const ITEMFINDER   ; 37
	const LIGHT_CLAY   ; 38
	const EXP_SHARE    ; 39
	const OLD_ROD      ; 3a
	const GOOD_ROD     ; 3b
	const DIVE_BALL    ; 3c
	const SUPER_ROD    ; 3d
	const PP_UP        ; 3e
	const ETHER        ; 3f
	const MAX_ETHER    ; 40
	const ELIXIR       ; 41
	const TIMER_BALL   ; 42
	const QUICK_BALL   ; 43
	const S_S_TICKET   ; 44
	const REPEAT_BALL  ; 45
	const WIDE_LENS    ; 46
	const ZOOM_LENS    ; 47
	const MOOMOO_MILK  ; 48
	const QUICK_CLAW   ; 49
	const PSNCUREBERRY ; 4a
	const AIR_BALLOON  ; 4b
	const SOFT_SAND    ; 4c
	const SHARP_BEAK   ; 4d
	const PRZCUREBERRY ; 4e
	const BURNT_BERRY  ; 4f
	const ICE_BERRY    ; 50
	const POISON_BARB  ; 51
	const KINGS_ROCK   ; 52
	const BITTER_BERRY ; 53
	const MINT_BERRY   ; 54
	const RED_APRICORN ; 55
	const TINYMUSHROOM ; 56
	const BIG_MUSHROOM ; 57
	const SILVERPOWDER ; 58
	const BLU_APRICORN ; 59
	const ICE_STONE    ; 5a
	const AMULET_COIN  ; 5b
	const YLW_APRICORN ; 5c
	const GRN_APRICORN ; 5d
	const CLEANSE_TAG  ; 5e
	const MYSTIC_WATER ; 5f
	const TWISTEDSPOON ; 60
	const WHT_APRICORN ; 61
	const BLACKBELT    ; 62
	const BLK_APRICORN ; 63
	const SILK_SCARF   ; 64
	const PNK_APRICORN ; 65
	const BLACKGLASSES ; 66
	const BLACK_SLUDGE ; 67
	const LINKING_CORD ; 68
	const FOCUS_SASH   ; 69
	const SMOKE_BALL   ; 6a
	const NEVERMELTICE ; 6b
	const MAGNET       ; 6c
	const MIRACLEBERRY ; 6d
	const PEARL        ; 6e
	const BIG_PEARL    ; 6f
	const EVERSTONE    ; 70
	const SPELL_TAG    ; 71
	const CHOICE_BAND  ; 72
	const CHOICE_SPECS ; 73
	const CHOICE_SCARF ; 74
	const MIRACLE_SEED ; 75
	const BIG_ROOT     ; 76
	const FOCUS_BAND   ; 77
	const X_SP_DEF     ; 78
	const ENERGYPOWDER ; 79
	const ENERGY_ROOT  ; 7a
	const HEAL_POWDER  ; 7b
	const REVIVAL_HERB ; 7c
	const HARD_STONE   ; 7d
	const LUCKY_EGG    ; 7e
	const SHELL_BELL   ; 7f
	const MACHINE_PART ; 80
	const EGG_TICKET   ; 81
	const LOST_ITEM    ; 82
	const STARDUST     ; 83
	const STAR_PIECE   ; 84
	const BASEMENT_KEY ; 85
	const PASS         ; 86
	const ZINC         ; 87
	const COVER_FOSSIL ; 88
	const PLUME_FOSSIL ; 89
	const CHARCOAL     ; 8a
	const BERRY_JUICE  ; 8b
	const SCOPE_LENS   ; 8c
	const CASTELIACONE ; 8d
	const ELECTRIZER   ; 8e
	const METAL_COAT   ; 8f
	const DRAGON_FANG  ; 90
	const MAGMARIZER   ; 91
	const LEFTOVERS    ; 92
	const MAGIC_RIBBON ; 93
	const RAZOR_CLAW   ; 94
	const RAZOR_FANG   ; 95
	const MYSTERYBERRY ; 96
	const DRAGON_SCALE ; 97
	const DUSK_STONE   ; 98
	const PP_MAX       ; 99
	const LIFE_ORB     ; 9a
	const NET_BALL     ; 9b
	const SACRED_ASH   ; 9c
	const HEAVY_BALL   ; 9d
	const SHINY_STONE  ; 9e
	const LEVEL_BALL   ; 9f
	const LURE_BALL    ; a0
	const FAST_BALL    ; a1
	const DUSK_BALL    ; a2
	const LIGHT_BALL   ; a3
	const FRIEND_BALL  ; a4
	const MOON_BALL    ; a5
	const LOVE_BALL    ; a6
	const NORMAL_BOX   ; a7
	const GORGEOUS_BOX ; a8
	const SUN_STONE    ; a9
	const ANCIENT_ORB  ; aa
	const EVIOLITE     ; ab
	const LIFT_KEY     ; ac
	const BERRY        ; ad
	const GOLD_BERRY   ; ae
	const SQUIRTBOTTLE ; af
	const DATA_DISK    ; b0
	const PARK_BALL    ; b1
	const FAIRY_FEATHER ; b2
	const RED_SHARD    ; b3
	const BLUE_SHARD   ; b4
	const GREEN_SHARD  ; b5
	const YELLOW_SHARD ; b6
	const MEMBERS_CARD ; b7
	const SOOTHE_BELL  ; b8
	const DAMP_ROCK    ; b9
	const HEAT_ROCK    ; ba
	const SMOOTH_ROCK  ; bb
	const ICY_ROCK     ; bc
	const ROCKY_HELMET ; bd
	const MUSCLE_BAND  ; be
	const WISE_GLASSES ; bf
	const EXPERT_BELT  ; c1
	const ITEM_FA      ; c2

add_tm: MACRO
if !DEF(TM01)
TM01 = const_value
	enum_start 1
endc
	define _\@_1, "TM_\1"
	const _\@_1
	enum \1_TMNUM
ENDM

; see data/moves/tmhm_moves.asm for moves
	add_tm GIGA_IMPACT  ; bf
	add_tm DRAGON_CLAW  ; c0
	add_tm SWORDS_DANCE ; c1
	add_tm CALM_MIND    ; c2
	add_tm WILD_CHARGE  ; c4
	add_tm TOXIC        ; c5
	add_tm HAIL         ; c6
	add_tm BULK_UP      ; c7
	add_tm VENOSHOCK    ; c8
	add_tm HIDDEN_POWER ; c9
	add_tm SUNNY_DAY    ; ca
	add_tm WILL_O_WISP  ; cb
	add_tm ICE_BEAM     ; cc
	add_tm BLIZZARD     ; cd
	add_tm HYPER_BEAM   ; ce
	add_tm LIGHT_SCREEN ; cf
	add_tm PROTECT      ; d0
	add_tm RAIN_DANCE   ; d1
	add_tm ENERGY_BALL  ; d2
	add_tm THUNDER_WAVE ; d3
	add_tm DAZZLINGLEAM ; d4
	add_tm SOLARBEAM    ; d5
	add_tm STONE_EDGE   ; d6
	add_tm THUNDERBOLT  ; d7
	add_tm THUNDER      ; d8
	add_tm EARTHQUAKE   ; d9
	add_tm RETURN       ; da
	add_tm DIG          ; db
	add_tm PSYCHIC_M    ; dd
	add_tm SHADOW_BALL  ; de
	add_tm BULLDOZE     ; df
	add_tm DOUBLE_TEAM  ; e0
	add_tm REFLECT      ; e1
	add_tm X_SCISSOR    ; e2
	add_tm FLAMETHROWER ; e3
	add_tm SLUDGE_BOMB  ; e4
	add_tm SANDSTORM    ; e5
	add_tm FIRE_BLAST   ; e6
	add_tm ROCK_TOMB    ; e7
	add_tm ACROBATICS   ; e8
	add_tm HEX          ; e9
	add_tm DREAM_EATER  ; ea
	add_tm SHADOW_CLAW  ; eb
	add_tm REST         ; ec
	add_tm ATTRACT      ; ed
	add_tm THIEF        ; ee
	add_tm FLASH_CANNON ; ef
	add_tm ROCK_SLIDE   ; f0
	add_tm STRUGGLE_BUG ; f1
	add_tm FOCUS_BLAST  ; f2
NUM_TMS = const_value - TM01 

add_hm: MACRO
if !DEF(HM01)
HM01 = const_value
endc
	define _\@_1, "HM_\1"
	const _\@_1
	enum \1_TMNUM
ENDM

	add_hm CUT          ; f3
	add_hm FLY          ; f4
	add_hm SURF         ; f5
	add_hm STRENGTH     ; f6
	;add_hm FLASH        ; f7
	;add_hm WHIRLPOOL    ; f8
	add_hm WATERFALL    ; f9
NUM_HMS = const_value - HM01

add_mt: MACRO
	enum \1_TMNUM
ENDM

	add_mt IRON_DEFENSE
	add_mt STEEL_WING
	add_mt IRON_TAIL
	add_mt ZEN_HEADBUTT
	add_mt FIRE_PUNCH
	add_mt OUTRAGE
	add_mt HONE_CLAWS
	add_mt SIGNAL_BEAM
	add_mt ICY_WIND
	add_mt AQUA_TAIL
	add_mt ICE_PUNCH
	add_mt DRAGON_PULSE
	add_mt BUG_BITE
	add_mt SYNTHESIS
	add_mt SEED_BOMB
	add_mt GUNK_SHOT
	add_mt GIGA_DRAIN
	add_mt DARK_PULSE
	add_mt LOW_KICK
	add_mt SUPER_FANG
	add_mt HYPER_VOICE
	add_mt EARTH_POWER
	add_mt THUNDERPUNCH
	add_mt DRAIN_PUNCH
NUM_TM_HM_TUTOR = __enum__ + -1

USE_SCRIPT_VAR EQU $00
ITEM_FROM_MEM  EQU $ff

; leftovers from red
SAFARI_BALL    EQU $08 ; MOON_STONE
MOON_STONE_RED EQU $0a ; BURN_HEAL
FULL_HEAL_RED  EQU $34 ; X_SPEED
